...
house = np.load('house.npy', allow_pickle=True).tolist()   # 导入哈希表
with open('addrs.txt', 'r', encoding='utf-8') as a:
    # addr.txt里面全都是已经写入本地的文件
    lines = a.readlines()
    for line in lines:
        line = line.replace('\n', '').split(' ')
        name = line[0]
...
table = idf(table)
print(table)
np.save('TFIDF-table.npy', table)
np.save('vocab.npy', np.array(vocab))
np.save('filekeys.npy', np.array(list(file_wordlist.keys())))
